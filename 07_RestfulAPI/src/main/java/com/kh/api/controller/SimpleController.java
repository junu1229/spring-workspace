 ...
 * - 하나의 URI는 하나의 고유한 리소스(Resource)를 대표하도록 설계하는 전송 방식
 ...
 * - 메서드의 리턴 타입으로 사용자가 정의한 클래스 타입도 사용할 수 있고, 이 외에도 여러 타입으로 보내는데 이를 JSOM이나 XML로 자동 처리
 * 
...
public class SimpleController {
	...
	// http://localhost:8081/api/simple/hello
	@GetMapping("/hello")
	...
	public List<Person> allGreet() {
		...
		for(int i=0; i<5; i++) {
			...
			person.setName("금요일" + i);
			person.setMessage("아직아님" + i);
			list.add(person);
		}
