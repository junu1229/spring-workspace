...
- 동적인 웹 어플리케이션을 개발하기 위한 여러 가지 서비스를 제공하고 있으며 대한민국 공공기관의 웹 서비스 개발 시 사용을 권장하고 있는 전자정부표준 프레임워크의 기반 기술로서 쓰이고 있다.
...
- JDBC나 데이터베이스에 연결하는 계층으로, 트랜잭션에 해당하는 기능을 담당하여 영속성 프레임워크(EX: MyBatis)와 연결을 담당한다.
...
- 스프링 애플리케이션 컨텍스트에서 빈을 로드하고 이 컨텍스트에 있는 빈(Bean)을 이용하여 테스트 코드를 작성할 수 있다.